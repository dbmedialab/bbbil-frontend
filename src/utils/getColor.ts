...
type Shade = 'dark' | 'light' | 'lighter' | ''
const getColor =
  (color: string, shade: Shade = '') =>
  (props: { theme: { colors: { [x: string]: any; skinColors: { [x: string]: any } } } }) => {
    const colorString = props.theme.colors.skinColors[color]
      ? props.theme.colors.skinColors[color]
      : color
    const capShade: string = shade !== null ? shade.slice(0, 1).toUpperCase() + shade.slice(1) : ''
    const shadedColorString =
      shade !== null ? unshadeColorString(colorString) + capShade : colorString

    return props.theme.colors[shadedColorString]
  }

const unshadeColorString = (color: string) =>
  (color || '')
    .replace(/Dark$/, '')
    .replace(/Light$/, '')
    .replace(/Lighter$/, '')
